...
public:
    ...
    inline Bitmap(int width, int height, uint32_t *in) : _w(width), _h(height)
    {
        _pitch = width * 4;
        _bits = new uint8_t[_pitch * _h];
        memcpy(_bits, in, _pitch * _h);
        FlipVertical();
        for (int i = 0; i < _pitch * _h; i += 4)
        {
...
public:
    ...
    inline bool SaveFile(std::string filename, bool withAlpha = false) const
    {
